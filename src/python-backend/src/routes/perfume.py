...
async def get_perfumes_by_id(perfume_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    if perfume is None:
        raise HTTPException(
            status_code=404, detail=f"Perfume with ID: {perfume_id} not found"
        )
    return perfume


@router.post("/", response_model=schemas.PerfumeResponse, status_code=201)
def create_perfume(
...
async def update_perfume(
    ...
    perfume_query = (
        db.query(models.Perfume).filter(models.Perfume.id == perfume_id).first()
    )
